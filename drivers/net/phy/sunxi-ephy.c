...
static struct phy_driver sunxi_phy_drivers[] = {
	{
		.phy_id = 0x00441400,
		.name = "ephy",
...
static int ephy_init(void)
{
	...
	ret = phy_drivers_register(sunxi_phy_drivers, ARRAY_SIZE(sunxi_phy_drivers), THIS_MODULE);
	if (ret)
		goto phy_drivers_error;
...
{
	phy_drivers_unregister(sunxi_phy_drivers, ARRAY_SIZE(sunxi_phy_drivers));
	platform_driver_unregister(&ac200_ephy_driver);
}
